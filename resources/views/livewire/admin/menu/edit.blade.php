...
@section('content')
    ...
    <section class="w-100 d-flex justify-content-center align-items-center">
        <div>
            <form action="{{ route('admin.menu.update', $menu) }}" class="form-control" method="post">
                @csrf
                @method('put')
                <div class="border-bottom border-1 border-shades-1 mb-4">
                    <h3 class="fs-4 text-center text-shades-1">ویرایش منو</h3>
                </div>
                <div>نام منو :</div>
                <div class="input-group mb-2">
                    <input class="form-control rounded-2 border-1" value="{{ old('name', $menu->name) }}" name="name" type="text" placeholder="نام منو">
                    @error('name')
                ...
                <div>آدرس منو :</div>
                <div class="input-group mb-2">
                    <input class="form-control rounded-2 border-1" value="{{ old('url', $menu->url) }}" name="url" type="text">
                    @error('url')
                    <span class="alert-red py-1" role="alert">
                            <strong class="text-red">
                                {{ $message }}
