...
@section('content')
    <section class="w-100 d-flex justify-content-center align-items-center">
        <div>
            <form action="{{ route('admin.faq.store') }}" class="form-control" method="post">
                @csrf
                <div class="border-bottom border-1 border-shades-1 mb-4">
                    <h3 class="fs-4 text-center text-shades-1">افزودن سوال متداول</h3>
                </div>
                <div>سوال :</div>
                <div class="input-group mb-2">
                    <input class="form-control rounded-2 border-1" type="text" name="question" value="{{ old('question') }}" placeholder="سوال خود را بنویسید">
                    @error('question')
                ...
                <div>پاسخ :</div>
                <div class="mb-6">
                    <textarea name="answer" id="" cols="30" rows="10" class="form-control border-1" placeholder="جواب سوال">{{ old('answer') }}</textarea>
                    @error('answer')
                         <span class="alert-red py-1" role="alert">
                                <strong class="text-red">
                                    {{ $message }}
