...
class LoginRegisterController extends Controller
{
    ...
    {
        return view('livewire.customer.auth.login-register');
    }
    ...
    public function loginRegister(LoginRegisterRequest $request)
    {
        ...
        if ($type == 0) {
            ...
            $data = array(
                'from' => '50002710071426',
                'to' => $user->mobile,
                'text' => "کد تایید : $otp_code\nبه سامانه مجموعه ابزار کد خوش آمدید ");
...
//
        ...
        elseif ($type == 1) {
            ...
            $emailService->setFrom('AbzarCode@example.com', 'مجموعه ابزار کد');
...
//        $messageService->send();
    ...
    public function loginConfirmForm($token)
    {
        ...
        return view('livewire.customer.auth.login-confirm', compact('token', 'otp'));
    }

    public function loginConfirm($token, LoginRegisterRequest $request)
    {
        $inputs = $request->all();
        $otpCode = $inputs['otp_one'] . $inputs['otp_two'] . $inputs['otp_three'] . $inputs['otp_four'] . $inputs['otp_five'] .$inputs['otp_six'];
        $otp = Otp::where('token', $token)->where('use', 0)->where('created_at', '>=', Carbon::now()->subMinute(2)->toDateTimeString())->first();
        if (empty($otp)) {
    ...
    {
        $otp = Otp::where('token', $token)->where('created_at', '<=', Carbon::now()->subMinutes(3)->toDateTimeString())->first();
        if (empty($otp)) {
...
//
        ...
        elseif ($otp->type == 1) {
            ...
            $emailService = new EmailService();
            $details = [
                'title' => "مجموعه ابزار کد|ایمیل فعال سازی",
                'body' => "کد فعال سازی شما : $otp_code ",
            ];
            $emailService->setDetails($details);
            $emailService->setFrom('no-reply@example.com', 'مجموعه ابزار کد');
            $emailService->setSubject("کداحراز هویت");
